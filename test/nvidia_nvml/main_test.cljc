(ns nvidia_nvml.main-test
  "Contract + behavioral test for the nvidia_nvml-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "nvidia_nvml-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'nvidia_nvml.main-test)]
