(ns nvidia_nvml.main
  "Kotodama WASM entrypoint for the Nvidia_nvml clean-room actor (L4) — Clojure port.
  ...
  facts (`nvidia_nvml.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nvidia_nvml")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nvidia_nvml.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nvidia_nvml-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nvidia_nvml-compat/ready)
